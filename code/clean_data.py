            ...
            for item in other_lists:
                if isinstance(item, str): 
                    ...
                    if "full-time" in item.lower():
                        full_time = 1
            return Internship_flag, on_site_flag, remote_flag, Hybrid_flag, part_time,full_time
        else:
            return 0,0,0,0,0,0
...
def data_clean(files):
    ...
    cleaned_data['internship_flag'] = change_location['others'].apply(lambda x: check_keywords(x)[0]) 
    cleaned_data['on_site_flag'] = change_location['others'].apply(lambda x: check_keywords(x)[1])
    cleaned_data['remote_flag'] = change_location['others'].apply(lambda x: check_keywords(x)[2])  
    cleaned_data['Hybrid_flag'] = change_location['others'].apply(lambda x: check_keywords(x)[3])
    cleaned_data['part_time'] = change_location['others'].apply(lambda x: check_keywords(x)[4])  
    cleaned_data['full_time'] = change_location['others'].apply(lambda x: check_keywords(x)[5])

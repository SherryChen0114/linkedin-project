...
def job_ave_salary(dataframe):
    ...
    filtered_data = dataframe[
        (dataframe["salary_yr"] > 0) & (dataframe["full_time"] == 1)
    ]
...
def average_salary_by_state(dataframe):
    ...
    avg_salary_by_state = (
        valid_states_data.groupby("state")["salary_yr"]
        .mean()
...
def get_skill_count(job, df):
    ...
    word_counts = dict()
    all_skills = (
        df["skills"] if job == "all" else df.loc[df["search_key"] == job, "skills"]
    )
...
if __name__ == "__main__":
    ...
    write_csv(
        os.path.join(OUTPUT_DIR, "search_key_counts.csv"),
        search_key_counts.reset_index(),
    )
    plot_search_key_counts(search_key_counts)

    average_salary = job_ave_salary(cleaned_data)
    write_csv(
        os.path.join(OUTPUT_DIR, "average_salary_by_job.csv"),
    ...
    write_csv(
        os.path.join(OUTPUT_DIR, "average_salary_by_arrangement.csv"),
        avg_salaries.reset_index(),
    )
    plot_salary_by_work_arrangement(avg_salaries)

    avg_salary_by_state = average_salary_by_state(cleaned_data)
    write_csv(
        os.path.join(OUTPUT_DIR, "average_salary_by_state.csv"),
        avg_salary_by_state.reset_index(),
    )

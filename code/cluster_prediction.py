...
def generate_classify_variables(based_df, classify_df):
    ...
    df[["professional", "soft", "coding"]] = based_df["skills"].apply(
...
def find_major_cluster_per_search_key(df):
    ...
    cluster_distribution = (
        df.groupby(["search_key", "cluster"]).size().unstack(fill_value=0)
    )
...
if __name__ == "__main__":
    ...
    classified_skills = read_classified_skills(
        os.path.join(OUTPUT_DIR, "classified_Skills.csv")
    )
